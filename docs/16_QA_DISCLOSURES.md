...
| **Format** | PostgreSQL rows (buoys, current observations, buoy contacts, vessel trips, incidents; plus legacy pressure rows from the retired buoy-barometer design) |
...
**Say this plainly:** the current field and vessel trips are simulated. Squall
alerts come from the PAGASA weather API, not from our own sensors; the buoys
carry no barometer.
...
- **PAGASA weather API → squall alerts.** No buoy barometer. PAGASA advisories
  covering the fishing grounds become RETURN NOW alerts over the radio path.
