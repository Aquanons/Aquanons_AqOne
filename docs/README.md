...
Last checked: 2026-09-26 22:50 +08:00.
...
| [Plan 71: dashboard drift and trip-anomaly render fixes](71_DASHBOARD_DRIFT_TRIP_RENDER_FIXES_IMPLEMENTATION_PLAN.md), Rev 2 | **ACTIVE** (`auto` mode), approved by Len 2026-09-26. The 2026-09-26 render check ([audit](audits/DASHBOARD_DRIFT_TRIP_RENDER_AUDIT_2026-09-26.md)) found 11 defects: the Trip Checks tab is off-screen, trip checks show no reason, fake overdue vessels, an invisible synthetic-replay badge, no drift legend, overstated contour masses, and demo beats 5 and 6 plus the seed data crash. Phases 1-3 complete 2026-09-26; Phase 4 (drift card and map explain themselves) next. | Len approves and owns the web paths for this plan (Arnold, the usual dashboard owner, is busy; he is told of the `docs/05` anomaly factor change). Implementer: Claude Code. Worktree `../AqOne-render` (`fix/dashboard-render`) holds the local `HANDOFF.md`. Owned paths: the plan's "Owned paths" section (backend anomaly, drift, SOS, demo and generator modules; `web/js/dashboard*`, `web/css/dashboard.css`, `web/html/dashboard.html`; `tools/render-check/`; `docs/render-fixes/`). | Run Phases 1-7 back to back; each verified phase merges to `master` and is pushed. Stop only on a failed gate or a decision that is Len's. |
| [Plan 72: toggleable dashboard demo mode](72_DASHBOARD_DEMO_MODE_IMPLEMENTATION_PLAN.md), Rev 1 | **DRAFT**, not started. Council deliberation recorded in the plan: one demo data source behind `authFetch`, all-demo or all-live per tab, no `/api/*` calls in demo mode, an always-visible banner. | Len owns approval. No implementer or worktree assigned; no paths claimed. | Len answers Q1-Q5. Starts after Plan 71 Phase 3; should land before RSTW (2026-10-01) to be useful there. |
...
| Draft plans not yet started | `67` (stagnant mode), `72` (dashboard demo mode) |
