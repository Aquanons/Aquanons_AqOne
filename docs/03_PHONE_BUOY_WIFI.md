...
`POST /v1/sos` adds one optional field:

| Field | Required | Notes |
| --- | --- | --- |
| `nonce` | no | Integer `0..4294967295`, generated by the phone once per incident and reused on every retry and every transport (`docs/04` E4.1). |

- The phone may send `nonce` before any pod or backend change: the pod ignores unknown JSON keys, and the backend ignores unknown fields (docs/62 Section 1.2 item 4).
- `note` (64 bytes) and `boat` (32 bytes) are limits in UTF-8 **bytes**, and the phone cuts them only on a character boundary.

### E3.2 What `accepted: true` means

`accepted: true` means the SOS is **queued on the pod**, and nothing more.
It is not delivered, and the phone must not treat it as delivered.
The phone keeps trying the direct internet path until the backend confirms `delivered` (`docs/06` "Edge-case remediation contract").

### E3.3 Pod duplicates and status (Track F phase F1)

After F1 merges:

- When a handoff arrives for a `(vessel_id, nonce)` already in the pod's queue, the pod fills that queued call's empty `note`, `lat` or `lon` from the new handoff instead of dropping it (EC-H5, EC-H1).
  It never overwrites a field that already has a value.
- `GET /v1/sos/status?vessel_id=<id>` returns `nonce` and `resolution_code` for each tracked call, with `resolution_code` meaning what `docs/05` E5.1 says.

Per-client queue caps, pod pairing and per-pod passphrases are defined in Track F's own contract step (docs/62D Phase F0), not here.
