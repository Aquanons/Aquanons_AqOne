...
|---|---|---|
| Mobile app | 🟡 Built and automatically tested | On 2026-09-26 `flutter analyze` reported no issues and 379 tests passed. Delivery keeps retrying until the backend confirms it, a foreground service holds a pending SOS, and MDRRMO enrolment, confirmed stand-down and a silent-SOS setting are in. The fisher-friction work (`docs/64`, `docs/65`) has made the SOS status honest (never "sent" before a pod has it), fixed a countdown that could freeze without sending, and made text and icons readable in sun with four labelled destinations. No release APK is tracked in the repository, and the device tests (foreground service, silent SOS, reinstall identity, countdown double tap) have not been run. |
...
| **Squall alerts** | PAGASA weather API relay (not a model) | PAGASA forecast areas covering the fishing grounds; PAGASA's lead time | PAGASA thunderstorm, rainfall and wind advisories; Open-Meteo marine data | No buoy barometer. The earlier `aqone-squall-v2` pressure model belongs to a dropped design; PAGASA API access and terms still to be confirmed. |
